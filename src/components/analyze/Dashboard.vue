<template>
  <section class="dashboard" v-if="content">
    ...
    <h3 v-if="active.mentions_total">Menções Totais: <span>{{ formatNumber(active.mentions_total) }} <template v-if="active.mentions_increased">({{ active.mentions_increased > 0 ? '+' : ''}}{{ active.mentions_increased }}%)</template></span></h3>
    <div class="dashboard__robots" v-if="active.robot_count">
      ...
      <span class="value">{{ formatNumber(active.robot_count) }} <template v-if="active.robot_increased">({{ active.robot_increased > 0 ? '+' : ''}}{{ active.robot_increased }}%)</template></span>
    </div>
  </section>
</template>
